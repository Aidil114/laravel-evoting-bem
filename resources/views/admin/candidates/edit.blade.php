...
<div class="max-w-3xl mx-auto bg-white p-6 rounded-lg shadow">
    <h2 class="text-xl font-bold mb-4">Edit Kandidat</h2>
    ...
    <form action="{{ route('candidates.update', $candidate) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" value="{{ $candidate->name }}" class="w-full border p-2 rounded" required>
        </div>

        <fieldset class="mb-4">
            <div class="font-medium mb-2">Ganti Foto Kandidat</div>
            <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center cursor-pointer relative hover:border-blue-500 transition"
                ...
                <div class="mt-4">
                    @if($candidate->photo)
                        <img id="preview" src="{{ asset('storage/'.$candidate->photo) }}" class="mx-auto max-h-40 rounded">
                    @else
                        <img id="preview" class="mx-auto max-h-40 hidden rounded">
                    @endif
                </div>
            </div>
            @error('photo')
        ...
        <div class="mb-4">
            ...
            <textarea name="visi" class="w-full border p-2 rounded" rows="3">{{ $candidate->visi }}</textarea>
        </div>

        <div class="mb-4">
            <label class="block mb-2 font-medium">Misi</label>
            <textarea name="misi" class="w-full border p-2 rounded" rows="3">{{ $candidate->misi }}</textarea>
        </div>
